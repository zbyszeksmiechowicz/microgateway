...
const startServer = function startServer(keys, pluginDir,config, cb) {
  ...
  if (config.heapDumpInterval) {
    console.log("Starting heapdump timer for process " + process.pid);
    var heapdump = require('heapdump');
    setInterval(
      () => heapdump.writeSnapshot('/tmp/' + process.pid + '-' + Date.now() + '.heapsnapshot'),
    1000*config.heapDumpInterval)
  }
