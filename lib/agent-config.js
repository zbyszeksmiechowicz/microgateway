...
const startWithConfig = function getConfigStart(options, cb) {
  startServer(options.pluginDir, JSON.parse(process.env.CONFIG), cb);
};

const startServer = function startServer(pluginDir,config, cb) {
  agent.start(pluginDir, config, function (err) {
    cb(err, agent, config);
  });
}
