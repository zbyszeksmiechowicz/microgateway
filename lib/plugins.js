 ...
 */
const Plugins = function (gateway, pluginDir) {
...
Plugins.prototype.loadPlugins = function (config) {
  ...
  const sequence = _reorderPlugins(combinedDirs, config.plugins);
  ...
  sequence.forEach(function (pluginName) {
    ...
    gateway.addPlugin(config, pluginName, _.bind(plugin.init, plugin));
  });

}


function _reorderPlugins(dirs, sequence) {
  const analytics = 'analytics-apid';
...
module.exports = function (gateway, pluginDir) {
  return new Plugins(gateway, pluginDir);
};
